...
def train(net, global_net, previous_net, trainloader, learning_rate, mu, temp, epochs: int):
    ...
    # def cos(pro1, pro2):
    #     return np.dot(pro1, pro2) / (np.linalg.norm(pro1) * np.linalg.norm(pro2))

    for epoch in range(epochs):
        epoch_loss_collector, epoch_loss1_collector, epoch_loss2_collector = [], [], []
        ...
        print(f"Total loss {epoch_loss}, current model loss {epoch_loss1}, previous model loss {epoch_loss2}")

        print(f"Epoch {epoch + 1}: train loss {epoch_loss_train}, macro F1 {macro_f1}, micro F1 {micro_f1}")
...
class FlowerNumPyClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        if not os.path.exists(os.path.join("../baselines/store_weights", f"{self.cid}", "prev_net.pt")):
            prev_net = copy.deepcopy(self.net)
        else:
            # load previous model from dir
            prev_net.load_state_dict(torch.load(os.path.join("../baselines/store_weights", f"{self.cid}", "prev_net.pt")))
        ...
        if not os.path.exists(os.path.join("../baselines/store_weights", f"{self.cid}")):
            os.makedirs(os.path.join("../baselines/store_weights", f"{self.cid}"))
        torch.save(self.net.state_dict(), os.path.join("../baselines/store_weights", f"{self.cid}", "prev_net.pt"))
