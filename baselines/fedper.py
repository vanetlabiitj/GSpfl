...
class Net(nn.Module):
    def __init__(self, num_inputs=8, hidden_units=16):
        ...
        self.personalize = nn.Sequential(
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_units, out_features=8),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        _, (hn, _) = self.shared(x, (h0, c0))

        out = self.personalize(hn[0])
        return out
...
def set_parameters(net, parameters: List[np.ndarray]):
    # Update shared layers with global model parameters
    params_dict = zip(net.shared.state_dict().keys(), parameters)
    state_dict = OrderedDict({k: torch.Tensor(v) for k, v in params_dict})
    net.shared.load_state_dict(state_dict, strict=True)


def get_parameters(net) -> List[np.ndarray]:
    # share global model parameters (shared layers)
    return [val.cpu().numpy() for _, val in net.shared.state_dict().items()]
...
class FlowerNumPyClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # os.makedirs("../per_Fed/store_weights", exist_ok=True)
        # file_path_weights = os.path.join("../per_Fed/store_weights", f"client_{self.cid}_model.pth")
        # torch.save(self.net.personalize.state_dict(), file_path_weights)
...
class FedCrime(fl.server.strategy.Strategy):
    ...
    def aggregate_fit(
        ...
        #aggregation_rule_kwargs = num_clients_to_keep
