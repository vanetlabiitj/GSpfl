...
def load_datasets():
    ...
    for partition_id, partition_df in group_partition:
      if partition_id in [3, 15, 10, 58, 49, 20, 30, 32, 75, 56]:
...
def train(net, trainloader, learning_rate, epochs: int):
    ...
    for epoch in range(epochs):
        ...
        return epoch_loss_train, micro_f1, macro_f1, local_steps, weight
...
class FlowerNumPyClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        train_loss, train_micro, train_macro, local_norm, weight = train(self.net, self.trainloader, learning_r, epochs=5)
        ...
        return get_parameters(self.net), len(self.trainloader), {"loss": float(train_loss), "micro": float(train_micro),
                                                                 "macro": float(train_macro), "local_norm": local_norm, "weight": weight}
...
class FedCrime(fl.server.strategy.Strategy):
    ...
    def aggregate_fit(
        ...
        local_tau = [res.metrics["local_norm"] for _, res in results]
        ...
        for _client, res in results:
            ...
            print(f"client {_client} and scale is {scale}" )

            aggregate_parameters.append((params, scale))



        parameters_aggregated = ndarrays_to_parameters(aggregate(aggregate_parameters))
