...
from flwr.server.strategy.aggregate import aggregate, weighted_loss_avg, aggregate_median, aggregate_krum, aggregate_trimmed_avg, aggregate_bulyan
...
def train(net, personalize_param, global_param, trainloader, learning_rate, server_round, cid, epochs: int):
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch + 1}: train loss {epoch_loss}, accuracy {epoch_acc}, macro F1 {macro_f1}, micro F1 {micro_f1}")
...
class FlowerNumPyClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        train_loss, train_micro, train_macro = train(self.net, personalize_param, global_param, self.trainloader, learning_r, server_round, self.cid, epochs=5)
...
def evaluate(
    ...
    for batch_idx, globaltestloader in enumerate(globaltestloaders):
        loss, accuracy, micro, macro, classreport = test(net, globaltestloader)
        losses.append(loss)
...
class FedGCNStrategy(fl.server.strategy.Strategy):
    ...
    def aggregate_fit(
        ...
        # construct model from flatten weights
